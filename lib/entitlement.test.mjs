...
test("canAccessPersonalizedDashboard allows any active full-skill subscription", () => {
  ...
  assert.equal(isDiagnosticComplete(emptyProfile), false);
  assert.equal(canAccessPersonalizedDashboard(emptyProfile, sub), true);
});

test("canAccessPersonalizedDashboard requires diagnostic when not subscribed", () => {
  const inactiveSub = { is_active: false, plan_slug: FULL_SKILL_PROGRAM_SLUG };
  const emptyProfile = {
    source_counts: { diagnostic: 0 },
    module_summary: {},
  };

  assert.equal(canAccessPersonalizedDashboard(emptyProfile, inactiveSub), false);
  assert.equal(
