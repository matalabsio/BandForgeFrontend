...
function postCheckoutDestination(sub, opts = {}) {
  ...
  if (receiptSlug === FULL) return "/dashboard?activating=1";
  if (receiptSlug === DUAL) return "/practice";
  if (receiptSlug === WRITING) return "/practice/writing";
  if (receiptSlug === SPEAKING) return "/practice/speaking";

  if (ent?.full_skill_program) return "/dashboard?activating=1";
  if (ent?.plans?.includes(DUAL)) return "/practice";
  if (ent?.writing_skill) return "/practice/writing";
  if (ent?.plans?.includes(SPEAKING) || ent?.speaking_skill) {
    return "/practice/speaking";
  }
  return "/pricing";
}
...
test("17. Stacked Writing + buy Speaking prefers receipt Speaking", () => {
  const sub = {
    entitlements: {
      plans: [WRITING, SPEAKING],
      writing_skill: true,
      speaking_skill: true,
      full_skill_program: false,
    },
  ...
  assert.equal(
    postCheckoutDestination(sub, { receiptPlanSlug: SPEAKING }),
    "/practice/speaking",
  );
  assert.equal(
    postCheckoutDestination(sub, { receiptPlanSlug: WRITING }),
    "/practice/writing",
  );
  // No receipt keeps Writing-first entitlement fallback.
  assert.equal(postCheckoutDestination(sub), "/practice/writing");
});
