...
test("abandonCheckoutResume clears pending, suppress, claim, and live attempt", () => {
  mockSessionStorage();
  globalThis.window = globalThis;
  resetCheckoutResumeStateForTests();
  ...
  assert.equal(isCheckoutResumeAutoOpenSuppressed(), false);
  assert.equal(isCheckoutResumeClaimed(), false);
  assert.equal(isCheckoutAttemptLive(), false);
});

test("module-live attempt survives claim clear and blocks remount re-entry", () => {
  resetCheckoutResumeStateForTests();
  markCheckoutAttemptLive();
  markCheckoutResumeClaimed();
  clearCheckoutResumeClaimed();
  assert.equal(isCheckoutAttemptLive(), true);
  assert.equal(isCheckoutResumeClaimed(), false);
  clearCheckoutAttemptLive();
  assert.equal(isCheckoutAttemptLive(), false);
});

test("shouldResumeDiagnosticCheckout requires checkout=1 and pending", () => {
  mockSessionStorage();
  resetCheckoutResumeStateForTests();
  globalThis.window = {
    location: {
      pathname: DIAGNOSTIC_RESULTS_PATH,
      search: "?checkout=1",
    },
  };
  assert.equal(shouldResumeDiagnosticCheckout(), false);
  setPendingCheckoutResume({ planSlug: "full_skill_program" });
  assert.equal(shouldResumeDiagnosticCheckout(), true);
  abandonCheckoutResume();
  assert.equal(shouldResumeDiagnosticCheckout(), false);
});

test("ensureDiagnosticCheckoutQueryIfPending does not mutate history", () => {
  mockSessionStorage();
  resetCheckoutResumeStateForTests();
  ...
  globalThis.window = {
    location: {
      pathname: DIAGNOSTIC_RESULTS_PATH,
      search: "",
      href: "http://localhost/diagnostic/results",
    },
    history: {
      replaceState: () => {
        replaceCalled = true;
  ...
  setPendingCheckoutResume({ planSlug: "full_skill_program" });
  // Pending without ?checkout=1 → resume false; must not rewrite URL.
  assert.equal(ensureDiagnosticCheckoutQueryIfPending(), false);
  assert.equal(replaceCalled, false);

  globalThis.window.location.search = "?checkout=1";
  assert.equal(ensureDiagnosticCheckoutQueryIfPending(), true);
  assert.equal(replaceCalled, false);
});
