...
import { RESULT_VIEWPORT_SCROLL_CLASS } from "@/modules/shared/components/result-page-viewport";
import { SectionResultsBrandBar } from "./section-results-brand-bar";
import { SectionResultsScrollContext } from "./section-results-scroll-context";
...
export function SectionResultsShell({
  ...
  useUnlockPageScroll(scrollRef, [children, footer, headerTitle, centered, card]);

  return (
    <SectionResultsScrollContext.Provider value={{ scrollRef }}>
      <div className="fixed inset-0 z-50 flex flex-col overflow-hidden bg-[#F4F7FB]">
        {showHeader ? (
        ...
        <div
          ref={scrollRef}
          className={`${RESULT_VIEWPORT_SCROLL_CLASS} ${centered ? "flex flex-col" : ""}`}
        >
          <div
    ...
    </SectionResultsScrollContext.Provider>
  );
}
