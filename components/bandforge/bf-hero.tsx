...
export function BandForgeHero() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set([desc, cta], { opacity: 0, y: 18 });
      ...
      tl.to(words, {
        ...
        .to(desc, { opacity: 1, y: 0, duration: 0.65, ease: "power3.out" }, 0.55)
        .to(cta, { opacity: 1, y: 0, duration: 0.65, ease: "power3.out" }, 0.75)
        .add(() => setSpotlightOn(true), 1.6);
  ...
  return (
    ...
    >
      ...
      <div
        ref={contentRef}
        className="bf-container pointer-events-none relative z-20 flex w-full flex-1 flex-col items-center justify-center will-change-transform"
      >
        <div className="mx-auto flex w-full max-w-2xl flex-col items-center justify-center px-4 text-center sm:px-6 lg:max-w-3xl">
          <h1
            id="bf-hero-heading"
          ...
          <p
            ref={descRef}
            className="relative z-20 mx-auto mt-5 max-w-[40ch] px-1 text-[0.8125rem] leading-[1.55] text-muted sm:mt-6 sm:max-w-[46ch] sm:text-[0.9375rem] sm:leading-[1.65] lg:mt-7 lg:max-w-[42ch] lg:text-base lg:leading-[1.65]"
          >
            A free diagnostic test that tells you exactly where you stand —
            across all four sections — in 90 minutes.
          </p>

          <div ref={ctaRef} className="pointer-events-auto relative z-20 mt-6 w-full sm:mt-7 lg:mt-8 lg:w-auto">
            <BfHeroActions />
          </div>
        </div>
      </div>
