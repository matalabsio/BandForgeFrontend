...
export function SpeakingSkillCourseHome({
  hubs,
  mockUnlock,
  highlightHubId = null,
  ...
  const { completed, total } = speakingCourseProgress(hubs, mockUnlock);
  const pct =
    total > 0 ? Math.min(100, Math.round((completed / total) * 100)) : 0;
  const currentHub = findCurrentSpeakingHub(hubs);
  const mockState = resolveSpeakingMockUiState(mockUnlock);
  const mocksLeft = speakingMocksRemaining(mockUnlock);

  const prefetchHrefs = useMemo(() => {
    const hrefs: string[] = [];
    if (currentHub) hrefs.push(`/practice/speaking/${currentHub.id}`);
    for (const h of hubs) {
      if (isSpeakingHubAccessible(h) && h.status !== "completed") {
        hrefs.push(`/practice/speaking/${h.id}`);
  ...
  return (
    <div className="relative space-y-6 pb-2 sm:space-y-8">
          ...
          <div className="flex min-w-0 items-start gap-3.5">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1.5 max-w-md text-[13px] leading-relaxed text-muted">
                Complete hubs in order across Part 1, Part 2, and Part 3.
                Progress and unlocks come from your Speaking Skill programme —
                review finished hubs anytime.
              </p>
            </div>
          </div>
          ...
          <div className="relative flex size-[4.5rem] shrink-0 items-center justify-center sm:size-20">
            ...
            <div className="text-center">
              <p
                ...
                data-testid="speaking-course-progress-completed"
              >
                {completed}
              </p>
              <p
                className="mt-0.5 text-[10px] font-semibold text-muted"
                data-testid="speaking-course-progress-total"
              >
                / {total}
              </p>
            </div>
        ...
        <div className="mt-5">
          ...
          <div className="h-2 overflow-hidden rounded-full bg-ink/[0.06]">
            <div
              ...
              style={{ width: `${pct}%` }}
              data-testid="speaking-course-progress-bar"
            />
          </div>
        </div>
            ...
            {currentHub ? (
              <>
                ...
                <span className="font-semibold">
                  {speakingHubDisplayTitle(currentHub, 1)}
                </span>
                )
              </>
            ) : null}
      ...
      {currentHub ? (
        ...
        >
          <Link
            href={`/practice/speaking/${currentHub.id}`}
            data-testid="speaking-course-continue-cta"
          ...
          >
            ...
            <div className="relative min-w-0">
              ...
              <p className="mt-1.5 flex flex-wrap items-center gap-x-2 text-[13px] text-white/65">
                ...
                {speakingHubPartLabel(currentHub) ? (
                  <>
                    <span>·</span>
                    <span>{speakingHubPartLabel(currentHub)}</span>
                  </>
                ) : null}
                <span>·</span>
      ...
      {hubs.length === 0 ? (
        <div
          data-testid="speaking-skill-empty-inventory"
          className="rounded-[28px] border border-white/60 bg-white/55 px-5 py-10 text-center backdrop-blur-xl"
        >
          <p className="text-sm text-muted">{speakingEmptyInventoryCopy()}</p>
        </div>
      ) : (
        <section
          ...
          aria-label="Speaking Skill hubs"
        >
          ...
          <ul className="grid gap-2.5 p-3 sm:grid-cols-2 sm:gap-3 sm:p-4 lg:grid-cols-3">
            {hubs.map((hub, index) => {
              ...
              const state = speakingHubUiState(hub, currentHub?.id);
              const accessible = isSpeakingHubAccessible(hub);
              const highlighted = highlightHubId === hub.id;
              const done = hub.status === "completed";
              const partLabel = speakingHubPartLabel(hub);
              const title = speakingHubDisplayTitle(hub, position);
              const cta = speakingHubCtaLabel(state);
              const lockedReason =
              ...
              return (
                <li
                  key={hub.id}
                  className="min-h-0"
                  data-testid={`speaking-hub-card-${hub.id}`}
                  data-hub-state={state}
                >
                  {accessible ? (
                    <Link
                      href={`/practice/speaking/${hub.id}`}
                      ref={(node) => {
...
function SpeakingSkillMockSection({
  mockUnlock,
  mockState,
  mocksLeft,
}: {
  mockUnlock: MockUnlock | null;
  mockState: ReturnType<typeof resolveSpeakingMockUiState>;
  ...
  if (unlocked) {
    body =
      mocksLeft != null
        ? `Speaking Mock ready · ${mocksLeft} remaining`
        : "Speaking Mock ready. Start when you are prepared.";
  } else if (used) {
    body = speakingMockUsedCopy();
  } else if (unavailable) {
    body = speakingMockUnavailableCopy();
  } else {
    body = speakingMockLockedCopy();
  }

  return (
    <div
      data-testid="speaking-skill-mock-section"
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        {unlocked && mockUnlock?.mock_test_id ? (
          <SpeakingSkillMockStartButton mockTestId={mockUnlock.mock_test_id} />
        ) : null}
      </div>
    </div>
  );
...
function SpeakingSkillMockStartButton({ mockTestId }: { mockTestId: string }) {
  ...
  async function onStart() {
    ...
    try {
      ...
      router.push(speakingSkillMockExamPath(mockTestId, res.part));
    } catch {
      setError(speakingMockUnavailableCopy());
      setBusy(false);
    }
  }
  ...
  return (
    <div className="flex w-full flex-col items-stretch gap-2 sm:w-auto">
      <button
        type="button"
        data-testid="speaking-skill-mock-start"
        disabled={busy}
        onClick={onStart}
