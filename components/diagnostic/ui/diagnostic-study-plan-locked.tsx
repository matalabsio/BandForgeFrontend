...
export function DiagnosticStudyPlanLocked({
  weeks,
  unlocked = false,
  onPurchase,
  purchaseBusy = false,
    ...
    <div className="relative overflow-hidden rounded-2xl border border-[#E8EDF3] bg-white shadow-[0_2px_12px_rgba(13,31,60,0.05)] sm:rounded-[18px]">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2.5 bg-[rgba(248,250,252,0.72)] px-6 backdrop-blur-[1px] sm:gap-[13px]">
        ...
        <p className="text-center text-[13.5px] font-semibold text-[#0D1F3C] sm:text-[15px]">
          Purchase your plan to unlock your personalised study plan
        </p>
        {onPurchase ? (
          <button
            type="button"
            onClick={onPurchase}
            disabled={purchaseDisabled || purchaseBusy}
            className={cn(bfPrimaryCtaNavClass, "mt-1 disabled:pointer-events-none disabled:opacity-60")}
          >
            {purchaseBusy ? "Opening checkout…" : "Unlock with Full Skill Program"}
          </button>
        ) : null}
      </div>
    </div>
